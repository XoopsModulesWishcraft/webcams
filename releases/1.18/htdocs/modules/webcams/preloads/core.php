					...
					switch($profile->getVar($xoModuleConfig['profile_field'])) {
						case $xoModuleConfig['profile_host_value']:
							...
							if ($xoModuleConfig['support_sexy']) {
								...
								$host->setVar('host', $GLOBALS['xoopsUser']->getVar('uname'));
								...
								if (!strpos($_SERVER['PHP_SELF'].(substr($_SERVER['QUERY_STRING'],0,1)=="?"?"":"?").$_SERVER['QUERY_STRING'], 'sexy/create.php?op=profile')) {
									$_SESSION['host_id'] = $host_id;
									xoops_loadLanguage('main', 'sexy');
									redirect_header(XOOPS_URL.'/modules/sexy/create.php?op=profile&host_id='.$host_id, 9, _SXY_MSG_CREATEPROFILE);
									exit(0);
								}
							}
							break;
