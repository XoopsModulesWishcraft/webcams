<?php
	...
	function WaitingHostHook($host) {
		...
		$xoopsMailer->setTemplate('webcams_host_waiting.tpl');
		$xoopsMailer->setSubject(_wbc_email_host_subject_waiting);
	...
	function WaitingStudioHook($studio) {
		...
		//$xoopsMailer->setHTML(true);
		$xoopsMailer->setTemplateDir($GLOBALS['xoops']->path('/modules/webcams/language/'.$GLOBALS['xoopsConfig']['language'].'/mail_templates/'));
		$xoopsMailer->setTemplate('webcams_studio_waiting.tpl');
		$xoopsMailer->setSubject(_wbc_email_studio_subject_waiting);
