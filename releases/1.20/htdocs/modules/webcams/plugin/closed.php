<?php
	...
	function ClosedHostHook($host) {
		...
		$xoopsMailer->setTemplate('webcams_host_closed.tpl');
		$xoopsMailer->setSubject(_wbc_email_host_subject_closed);
	...
	function ClosedStudioHook($studio) {
		...
		//$xoopsMailer->setHTML(true);
		$xoopsMailer->setTemplateDir($GLOBALS['xoops']->path('/modules/webcams/language/'.$GLOBALS['xoopsConfig']['language'].'/mail_templates/'));
		$xoopsMailer->setTemplate('webcams_studio_closed.tpl');
		$xoopsMailer->setSubject(_wbc_email_studio_subject_closed);
